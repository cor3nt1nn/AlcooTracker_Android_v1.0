...
class TauxAlc():
    ...
    def getSymptomes(self, username):
        ...
        symptomes = {(0.0, 0.09): "Aucun effet",
                     ...
                     (3.5, float("inf")): "Danger de Mort",
                     (4.0, float("inf")): "Intoxication élevée, Coma Ethylique"}
    ...
    def taux0(self, username, aff=True):
        ...
        if aff:
            if horZero > time.mktime(time.strptime(str(time.strftime("%b %d %H:%M %Y", time.localtime())),"%b %d %H:%M %Y")):
                phr= "Vous serez sobre à la date: "+ str(datetime.fromtimestamp(horZero))[:-3]
            else:
                phr= "Vous êtes sobre"
            return phr
        else:
    ...
    def conduiteaut(self, username):
        ...
        records.sort()
        if records==[]:
            phr= "Vous pouvez conduire, vous avez 0.0g d'alcool/L de sang"
            return phr
        else:
            ...
            if horaire<horMaxi and taux <= self.limit:
                phr = "Vous pouvez conduire, vous avez " + str(taux) + f"g d'alcool/L de sang mais vous n'avez PAS ENCORE atteint votre pic d'alcoolémie !\nVotre pic d'alcoolémie sera à {str(datetime.fromtimestamp(horMaxi))[11:-3]}"
                return phr

            elif taux <= self.limit:
                phr= "Vous pouvez conduire, vous avez " +str(taux)+"g d'alcool/L de sang"
                return phr
